...
using NetShopAPI.Features.Common.CentralizedErrorMessage;
...
namespace NetShopAPI.Features.Catalog.Commands.BulkCreatePositions
{
    public class BulkCreatePositionsHandler
    {
        ...
        public BulkCreatePositionsHandler(IUnitOfWork uow, IPositionRepository positions, ICategoryRepository categories)
        {
            _uow = uow;
            ...
            List<BulkCreatePositionsCommand> cmd, CancellationToken ct)
        {
            ...
            var existsNames = await _positions.GetExistingNamesAsync(validNames, ct);
