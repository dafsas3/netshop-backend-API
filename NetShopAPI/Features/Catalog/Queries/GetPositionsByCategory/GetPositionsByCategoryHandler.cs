...
namespace NetShopAPI.Features.Catalog.Queries.GetPositionByCategory
{
    public class GetPositionsByCategoryHandler
    {
        ...
        public async Task<Result<List<PositionResponse>>> Handler(int categoryId, CancellationToken ct)
        {
            ...
            var positionsDto = await _db.Positions.Where(p => p.Category.Id == categoryId)
